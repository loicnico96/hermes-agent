...
class TestPrimaryRuntimeSnapshot:
    ...
    def test_primary_anthropic_credential_rotation_updates_snapshot(self):
        ...
        assert agent._primary_runtime["api_key"] == "sk-ant-rotated-abcdef"
        assert agent._primary_runtime["anthropic_api_key"] == "sk-ant-rotated-abcdef"
...
class TestRestorePrimaryRuntime:
    ...
    def test_fallback_realigns_credential_pool_and_restore_recovers_primary_pool(self):
        ...
        with (
            ...
            patch.object(AIAgent, "_load_runtime_credential_pool", return_value=fallback_pool),
        ):
            agent._try_activate_fallback()
    ...
    def test_fallback_clears_credential_pool_when_target_provider_has_none(self):
        ...
        with (
            patch("agent.auxiliary_client.resolve_provider_client", return_value=(mock_client, None)),
            patch.object(AIAgent, "_load_runtime_credential_pool", return_value=None),
        ):
            agent._try_activate_fallback()
    ...
    def test_load_runtime_credential_pool_uses_custom_pool_key(self):
        custom_pool = _FakePool("custom")
        with (
            patch("agent.credential_pool.get_custom_provider_pool_key", return_value="custom:acme") as get_key,
            patch("agent.credential_pool.load_pool", return_value=custom_pool) as load_pool,
        ):
            result = AIAgent._load_runtime_credential_pool(
                "custom:acme",
                base_url="https://example.com/v1",
            )

        assert result is custom_pool
        get_key.assert_called_once_with("https://example.com/v1", provider_name="acme")
        load_pool.assert_called_once_with("custom:acme")

    def test_load_runtime_credential_pool_aligns_current_entry_to_runtime_credentials(self):
        pool = _make_credential_pool(
            "openrouter",
            [
                _make_pool_entry(
                    "entry-a",
        ...
        with patch("agent.credential_pool.load_pool", return_value=pool):
            result = AIAgent._load_runtime_credential_pool(
                "openrouter",
                base_url="https://openrouter.ai/api/v1",
                runtime_api_key="key-b",
            )

        assert result is pool
        assert pool.current() is not None
        assert pool.current().id == "entry-b"

    def test_load_runtime_credential_pool_reuses_current_pool_and_realigns_identity(self):
        pool = _make_credential_pool(
            "openrouter",
            [
                _make_pool_entry(
