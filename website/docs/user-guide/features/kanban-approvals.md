...
provide an alternative to using the Blocked column as indicating human review,
or using task runs as reviewers.
...
- **Approvals are structured decisions.**
  - Approvers MUST explicitly choose one of 3 defined decisions:
    ...
    - `"escalated"`: Task remains parked, human approval is requested.
...
Request an human approval when you want to guarantee the opportunity to manually inspect a task's result before it's moved to `"done"`.
...
When a task is in `"approval"`, the task's next state is determied from the attached approval rows in this order:
...
hermes kanban approval request t_abc --agent default --skill security-review
```
...
- **It is NOT possible to forcefully set an agent's decision.**
  - To bypass an agent's decision, unassign it, or issue an human decision.
...
This removes the approval row completely, as if it hadn't been requested (through existing task events/logs remain).
...
- If the agent is currently running, that process is killed.
- **If an human unassigns themselves after escalation, all `"escalated"` approvers are automatically re-requested.**
