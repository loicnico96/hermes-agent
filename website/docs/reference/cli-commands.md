...
Approval gates hang off the same command family via `hermes kanban approval ...` — for example `request`, `list`, `approve`, `reject`, `reset`, `reclaim`, and `remove`. See [Kanban task approvals](/docs/user-guide/features/kanban-approvals) for the approval lifecycle and CLI workflow.
