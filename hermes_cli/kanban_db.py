...
def _normalize_approval_optional_text(value: Optional[str]) -> Optional[str]:
    if value is None:
        return None
...
def _validate_approval_comment_id(
    ...
    if row is None:
        raise ValueError(f"comment_id {comment_id} does not belong to task {task_id}")
    return int(comment_id)


def _validate_approval_identity(
    *, approver_type: str, approver_profile: Optional[str], approver_skill: Optional[str]
    ...
    approver_profile = _normalize_approval_optional_text(approver_profile)
    approver_skill = _normalize_approval_optional_text(approver_skill)
    if approver_type == "human":
        if approver_profile is not None:
            raise ValueError("human approvals cannot set approver_profile")
        if approver_skill is not None:
    ...
    return approver_profile, approver_skill


def _validate_approval_status(status: str) -> str:
    if status not in VALID_APPROVAL_STATUSES:
        raise ValueError(f"status must be one of {sorted(VALID_APPROVAL_STATUSES)}")
    return status


def _validate_approval_run_status(status: str) -> str:
    if status not in VALID_APPROVAL_RUN_STATUSES:
        raise ValueError(
            f"status must be one of {sorted(VALID_APPROVAL_RUN_STATUSES)}"
        )
    return status
...
def create_task_approval(
    ...
    *,
    ...
    approver_profile, approver_skill = _validate_approval_identity(
        approver_type=approver_type,
        approver_profile=approver_profile,
        approver_skill=approver_skill,
    )
    status = _validate_approval_status(status)
    claim_lock = _normalize_approval_optional_text(claim_lock)
    last_failure_error = _normalize_approval_optional_text(last_failure_error)
    if consecutive_failures < 0:
        raise ValueError("consecutive_failures must be >= 0")
    with write_txn(conn):
        if get_task(conn, task_id) is None:
            raise ValueError(f"unknown task: {task_id}")
        comment_id = _validate_approval_comment_id(conn, task_id, comment_id)
        if approver_type == "human":
            ...
            duplicate = conn.execute(
                ...
                SELECT 1
                 ...
                 WHERE task_id = ?
                   ...
                   AND approver_skill IS ?
                """,
                (task_id, approver_profile, approver_skill),
            ).fetchone()
            if duplicate is not None:
                raise ValueError(
                    "task already has an agent approval for "
                    f"profile={approver_profile!r} skill={approver_skill!r}"
                )
        now = int(time.time())
        cur = conn.execute(
            """
...
def list_task_approvals(
    ...
    *,
    ...
    if approver_type is not None and approver_type not in VALID_APPROVAL_TYPES:
        raise ValueError(
...
def create_task_approval_run(
    ...
    *,
    ...
    claim_lock = _normalize_approval_optional_text(claim_lock)
    outcome = _normalize_approval_optional_text(outcome)
    error = _normalize_approval_optional_text(error)
    with write_txn(conn):
        ...
        if approval.approver_type != "agent":
            raise ValueError("approval runs are only valid for agent approvals")
        resolved_task_id = task_id or approval.task_id
        if resolved_task_id != approval.task_id:
            raise ValueError(
                f"approval {approval_id} belongs to task {approval.task_id}, not {resolved_task_id}"
            )
        comment_id = _validate_approval_comment_id(conn, resolved_task_id, comment_id)
        cur = conn.execute(
            """
            ...
            (
                int(approval_id),
                resolved_task_id,
                _normalize_approval_optional_text(profile) or approval.approver_profile,
                status,
                claim_lock,
                int(claim_expires) if claim_expires is not None else None,
                int(worker_pid) if worker_pid is not None else None,
                int(last_heartbeat_at) if last_heartbeat_at is not None else None,
                int(started_at) if started_at is not None else int(time.time()),
...
def get_task_approval_run(
    conn: sqlite3.Connection, approval_run_id: int
) -> Optional[ApprovalRun]:
    row = conn.execute(
        "SELECT * FROM task_approval_runs WHERE id = ?",
        (int(approval_run_id),),
    ).fetchone()
    return ApprovalRun.from_row(row) if row else None
...
def list_task_approval_runs(
    ...
    *,
    ...
    if approval_id is None and task_id is None:
        raise ValueError("approval_id or task_id is required")
    if status is not None:
        _validate_approval_run_status(status)
...
def reset_task_approval(conn: sqlite3.Connection, approval_id: int) -> bool:
    with write_txn(conn):
        cur = conn.execute(
            """
