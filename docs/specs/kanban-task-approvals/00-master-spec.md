...
Add first-class task approvals to Hermes Kanban so a task can complete execution, enter an explicit `approving` state, and be gated on one or more approval rows without overloading `blocked` or the existing ad-hoc `review-required` convention.
...
- approval rows are reset whenever a task moves to a status other than `approving`, `done`, or `archived`
...
- approval rows persist across task-state transitions; when a task moves into a status other than `approving`, `done`, or `archived`, those rows are reset to `requested` instead of being deleted

### 3.2 Task statuses

This slice adds `approving` to the task status set.

Meaning:
- `running`: implementation work is still in progress.
- `approving`: implementation work finished successfully, but the task is not yet complete because approval rows exist and are not fully resolved.
...
### 4.1 Entering `approving`

When a task worker finishes normally and the task has one or more approval rows, the task must transition to `approving` instead of `done`.
...
If a human approval row that is currently satisfying one or more `escalated` or `failed` agent approvals is removed while the task is still `approving`, the kernel must:
...
Rules:
- only valid for human approval rows in `requested`, `escalated`, or `failed`-blocking situations where human action is expected
...
- no existing task or run rows may be rewritten in place except for the additive task-status expansion to allow `approving`
